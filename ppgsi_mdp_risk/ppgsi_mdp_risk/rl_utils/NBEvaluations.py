...
def run_driving_license(dl, T, C, actions, model, model_name, **kwargs):
    if model_name == 'Neutral-VI' or model_name == 'Neutral-PI':
        ...
        rdl = model(dl, T, C, vl_slope1=slope1, vl_slope2=slope2,
                         num_actions=actions, epsilon=epsilon)
    elif model_name == 'CVAR_PECVaR':
        alpha, discount_factor, max_iter = kwargs['alpha'], kwargs['discount_factor'], kwargs['max_iter']
        rdl = model(dl, T, C, alpha=alpha, num_actions=actions, discount_factor=discount_factor, max_iter=max_iter)
